#!/usr/bin/env python3
"""
ICON MCP v108 MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('icon-mcp-v108_mcp')
...
logger.info("="*80)
logger.info(f"ICON MCP v108 MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://api.apis.guru/v2/")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("ICON MCP v108 MCP Server", host="0.0.0.0")
...
# - ICON_MCP_V108_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="icon-mcp-v108-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "icon-mcp-v108-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting ICON MCP v108 MCP Server")
    logger.info("="*80)
